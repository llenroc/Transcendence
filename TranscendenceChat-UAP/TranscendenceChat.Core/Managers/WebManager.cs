...
using TranscendenceChat.Core.Interfaces;

namespace TranscendenceChat.Core.Managers
{
    public class WebManager : IWebManager
    {
        ...
        public async Task<Result> PostData(Uri uri, MultipartContent header, StringContent content)
        {
            ...
            {
                throw new WebException("Transcendence Chat API Error: Service error", ex);
            }
        }
        ...
        public async Task<Result> GetData(Uri uri)
        {
            ...
            try
            {
                ...
                {
                    throw new WebException("Transcendence Chat API Error: Service not found.");
                }
                var responseContent = await response.Content.ReadAsStringAsync();
